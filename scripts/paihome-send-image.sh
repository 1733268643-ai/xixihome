#!/bin/bash
# Claude/Codex 从窗口向 PaiHome 对话发图。
# 用法: paihome-send-image.sh /绝对路径/图.jpg "配文（可选）" [window]
set -u
IMG="${1:?用法: paihome-send-image.sh <绝对路径> [配文] [win]}"
CAPTION="${2:-}"
WIN="${3:-$(tmux display-message -p '#S' 2>/dev/null || echo paihome)}"
...
req = urllib.request.Request(
    __import__('os').environ.get("PAIHOME_API_URL", "http://127.0.0.1:3001") + "/api/bridge/send-image",
