...
`scripts/` 中的 hook 模板默认工作在 tmux 会话 `xixihome`。部署前先配置 `backend/.env` 的 `BRIDGE_MACHINE_TOKEN`，然后按终端产品的 Hook 配置把相应脚本注册进去。

可用环境变量：

- `XIXIHOME_ROOT`：本交付包所在的绝对路径。
- `XIXIHOME_API_URL`：后端地址，默认 `http://127.0.0.1:3001`。
- `BRIDGE_TMUX_SESSION`：要同步到 XixiHome 的 tmux 会话名。
